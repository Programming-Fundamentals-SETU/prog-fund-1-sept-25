...
- A main method (public static void main(String[] args)) — the entry point only for Java applications that are run from the command line or an IDE.

**NOTE**
Not every class needs a main method. Only a program’s entry-point class requires main; other classes (like Book, Calculator, model objects or library classes) are used by those entry-point classes and don’t normally include main. 

In BlueJ you can also create and test objects directly without writing a main method.
