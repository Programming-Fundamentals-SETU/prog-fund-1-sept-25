...
public class Simulator
{
    ...
    // The probability that a scientist will be created in any given position.
    private static final double SCIENTIST_CREATION_PROBABILITY = 0.005;    
    ...
    private void populate()
    {
        ...
        for(int row = 0; row < field.getDepth(); row++) {
            for(int col = 0; col < field.getWidth(); col++) {
                if(rand.nextDouble() <= SCIENTIST_CREATION_PROBABILITY) {
                    Location location = new Location(row, col);
                    Actor scientist = new Scientist(location);
                    field.placeActor(scientist, location);
                }
